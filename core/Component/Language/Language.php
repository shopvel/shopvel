<?php
namespace Shopvel\Component\Language;

use Shopvel\Exception\LanguageException;
...
class Language
{
    ...
    /**
     * @return array
     * @throws \Shopvel\Language\LanguageException
     */
    public function all()
    {
        $languages = \Config::get('app.locales');
        $codes = explode(',', $languages);

        $output = [];
        foreach ($codes as $code) {
            $output[$code] = $this->get($code);
        }
        return $output;
    }
     ...
     */
    public function set($code, $codeIndex = 0, $outputIndex = 5)
    {
        $codes = $this->all();
        if (isset($codes[$code])) {
            \Session::put('shopvel.language', $code);
        } else {
            throw new LanguageException("Unkown language [" . $code . "]");
        }
    }
     ...
     * @throws LanguageException
     */
    public function get($code, $codeIndex = 0, $outputIndex = 5)
    {
        foreach ($this->languages as $lang) {
            if (!isset($lang[$codeIndex]) || !isset($lang[$outputIndex])) {
                throw new LanguageException("Unkown language index [" . $codeIndex . "]");
            }
            if ($lang[$codeIndex] == $code) {
                return $lang[$outputIndex];
            }
        }
        throw new LanguageException("Unkown language code [" . $code . "]");
    }
}